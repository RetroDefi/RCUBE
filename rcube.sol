...
contract RCUBE is Context, IBEP20, Ownable {
    ...
    string  private constant _NAME = 'RCUBE';
    string  private constant _SYMBOL = 'RCUBE';
    ...
    uint256 private _RCubeRotations = 0;
    uint256 private _qTradeRotations = 0;
    uint256 private _qBurnRotations = 0;
    uint256 private transferredRcube = 0;
    uint256 private countOfRCube = 0;
    ...
    function _rebase(uint256 supplyDelta) internal {
        _RCubeRotations = _RCubeRotations.add(1);
        _qTotal = _qTotal.add(supplyDelta);

        if(_RCubeRotations > 192 || _qTotal <= 8800000 * _CORRECTDECIMALS){
            _startEndProcedure();
        }
    }
